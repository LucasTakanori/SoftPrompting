...
class Trainer():
    ...
    def log_soft_prompt_visualization(self):
        if self.params.use_weights_and_biases:
            prompt_weights = self.net.visualize_soft_prompts()
            wandb.log({"soft_prompt_heatmap": wandb.Image(plt.imshow(prompt_weights))})
    ...
    def evaluate_training(self):
        ...
        with torch.no_grad():
            ...
            for batch_data in progress_bar:
                input_features, decoder_input_ids, labels = batch_data
                input_features = input_features.to(self.device)
                decoder_input_ids = decoder_input_ids.to(self.device)
                labels = labels.to(self.device)

                outputs = self.net(input_features, decoder_input_ids)
        ...
        with torch.no_grad():
            ...
            for batch_data in progress_bar:
                ...
                if isinstance(outputs, dict) and "generated_ids" in outputs:
                    ...
                    raise ValueError(f"Unexpected output format: {outputs.keys() if isinstance(outputs, dict) else type(outputs)}")
    ...
    def train_single_epoch(self, epoch):
        ...
        for self.batch_number, batch_data in enumerate(tqdm(self.training_generator, desc=f"Epoch {epoch}")):
            ...
            outputs = self.net(input_features, decoder_input_ids, labels)
            loss = outputs.loss
            ...
            loss.backward()
            grad_norm = torch.nn.utils.clip_grad_norm_(self.net.trainable_params, max_norm=1.0)
            self.optimizer.step()
    ...
    def train(self, starting_epoch, max_epochs):
        ...
        for self.epoch in range(starting_epoch, max_epochs):
            ...
            #self.log_soft_prompt_visualization()
