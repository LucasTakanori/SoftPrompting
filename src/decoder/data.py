...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Create decoder input by shifting the labels to the right
        decoder_input = F.pad(labels[:-1], (1, 0), value=self.processor.tokenizer.bos_token_id)

        # Apply soft prompt to decoder input
        if torch.rand(1) < self.prompt_use_rate:
            # Create a random soft prompt (this is just a placeholder, you might want to use a more sophisticated method)
            soft_prompt = torch.randint(0, self.vocab_size, (self.prompt_length,))
            decoder_input = torch.cat([soft_prompt, decoder_input])[:self.tokens_max_length]
